package cn.jsong.example.spring.consumer.aspect;
...
public class BindResultAspect {
	...
	@Pointcut("execution(public * con.jsong.example.spring.consumer.controller.*.*(..))")
		...
		for (Object arg : args) {
			if (arg instanceof BindingResult) {
				...
				if (result.hasErrors()) {
					...
					throw new ExampleException("1010000", result.getFieldError().getDefaultMessage());
				}
			}
		}
	}
